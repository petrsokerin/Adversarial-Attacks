...
@hydra.main(config_path='config', config_name='experiment_run', version_base=None)
def main(cfg: DictConfig):
    ...
    disc_model_check = load_disc_model(model_id=cfg['disc_check_params']['model_id'], 
                            path=cfg['disc_path'], 
    ...
    for alpha in tqdm(cfg['alphas']):
        ...
        if 'reg' in cfg['attack_type'] :
            ...
            attack_params['alpha'] = alpha
            attack_params['disc_models'] = load_disc_config(cfg['disc_path'], device, cfg['list_reg_model_params'])  
        ...
        aa_res_df, rej_curves_dict = ifgsm_procedure(model=model, loader=test_loader, criterion=criterion,
                                                    ...
                                                    eps_params=cfg['eps_params'], n_steps=cfg['n_iterations'],
